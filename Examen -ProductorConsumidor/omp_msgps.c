// Utiliza directivas críticas y atómicas para proteger secciones críticas.
#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
#include "queue.h"
...
#     pragma omp barrier //No dejes que ningún hilo envíe mensajes hasta que se construyan todas las colas
...
#     pragma omp atomic
      done_sending++;
#     ifdef DEBUG
      printf("Thread %d > terminado de enviar\n", my_rank);
#     endif

      while (!Done(msg_queues[my_rank], done_sending, thread_count))
         Try_receive(msg_queues[my_rank], my_rank);

      Free_queue(msg_queues[my_rank]);// Mi cola está vacía y todos terminaron de enviar
      free(msg_queues[my_rank]);//Por lo tanto, no se volverá a acceder a mi cola y está bien liberarla
   }  
...
void Usage(char *prog_name) {
   fprintf(stderr, "Uso: %s <numero de threads> <numero de mensages>\n",
         prog_name);
   fprintf(stderr, "   numero de mensages = numero enviado por cada thread\n");
   exit(0);
} 
...
#     pragma omp critical
      ...
      Dequeue(q_p, &src, &mesg);
   printf("Thread %d > recibido %d desde %d\n", my_rank, mesg, src);
} 
int Done(struct queue_s* q_p, int done_sending, int thread_count) {
   int queue_size = q_p->enqueued - q_p->dequeued;
