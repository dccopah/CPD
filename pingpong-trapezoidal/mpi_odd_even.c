...
//Llenar lista con entradas aleatorias
void Generate_list(int local_A[], int local_n, int my_rank) {
   int i;
...
//Imprimir línea de comando para iniciar el programa
void Usage(char* program) {
   ...
   fflush(stderr);
}  //Solo ejecuta el proceso 0

//Obtenga y verifique los argumentos de la línea de comando
void Get_args(int argc, char* argv[], int* global_n_p, int* local_n_p, 
         char* gi_p, int my_rank, int p, MPI_Comm comm) {
...
//proceso 0 lee la lista de stdin y la esparce a los otros procesos.
void Read_list(int local_A[], int local_n, int my_rank, int p,
         MPI_Comm comm) {
   int i;
   int *temp;
...
//Imprime el contenido de la lista global A
void Print_global_list(int local_A[], int local_n, int my_rank, int p, 
 ...
 //Compare 2 int utilizado qsort
int Compare(const void* a_p, const void* b_p) {
   int a = *((int*)a_p);
   int b = *((int*)b_p);

   if (a < b)
      return -1;// int es menor-> -1
   else if (a == b)
      return 0; //  int es igual -> 0
   else /* a > b */ // a mayor b
      return 1;
}  

//Ordena la lista local, (par-impar) para ordenar la lista global.
void Sort(int local_A[], int local_n, int my_rank, 
         int p, MPI_Comm comm) {
   int phase;
   int *temp_B, *temp_C;
   int even_partner;  //fase par->izquierda
   int odd_partner;   //fase impar-> derecha

   //Almacenamiento temporal utilizado en merge-split
   temp_B = (int*) malloc(local_n*sizeof(int));
   temp_C = (int*) malloc(local_n*sizeof(int));

   //Encontrar socios: rango negativo => no hacer nada durante la fase
   if (my_rank % 2 != 0) {
      even_partner = my_rank - 1;
      odd_partner = my_rank + 1;
      if (odd_partner == p) odd_partner = MPI_PROC_NULL;  // Inactivo durante la fase impar
   } else {
      even_partner = my_rank + 1;
      if (even_partner == p) even_partner = MPI_PROC_NULL;  // Inactivo durante la fase par
      odd_partner = my_rank-1;  
   }

// Ordenar la lista local usando ordenación rápida incorporada
...
//Una iteración del tipo de transposición pares pares
void Odd_even_iter(int local_A[], int temp_B[], int temp_C[],
        int local_n, int phase, int even_partner, int odd_partner,
        int my_rank, int p, MPI_Comm comm) {
   MPI_Status status;
 ...
 //Junta los elementos local_n más pequeños en my_keys 
 //y recv_keys en temp_keys. Luego copia temp_keys de nuevo 
 //en my_keys.
void Merge_low(
      int  my_keys[],     /* in/out    */
      int  recv_keys[],   /* in        */
 ...
 //Fusiona los elementos local_n más grandes en local_A y temp_B en temp_C. 
 //Luego copie temp_C nuevamente en local_A.
void Merge_high(int local_A[], int temp_B[], int temp_C[], 
        int local_n) {
   int ai, bi, ci;
