 //Esta versión usa el tokenizador seguro para subprocesos strtok_r
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <omp.h>
...
//Leer texto y almacenar como una matriz de cadenas, una por línea de texto de entrada
void Get_text(char* lines[], int* line_count_p) {
...
//Tokeniza las lineas de entrada
void Tokenize(
      char*  lines[], 
      int    line_count, 
      int    thread_count) {
...
#     pragma omp for schedule(static, 1)
      for (i = 0; i < line_count; i++) {
         printf("Thread %d > linea %d = %s", my_rank, i, lines[i]);
         j = 0; 
      ...
      if (lines[i] != NULL) 
         printf("Thread %d > Después de tokenizar, mi línea = %s\n",
            my_rank, lines[i]);
      } 
   }  
